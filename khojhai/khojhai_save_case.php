...
$qry0 = mysqli_query($con, "select * from cases where id = '$get_id' LIMIT 1");
...
$qry1 = mysqli_query($con, "SELECT * FROM case_followers WHERE case_id = '$case_id' AND follower_id = '$follower_id'");
	
$qry_count1 = mysqli_num_rows($qry1);
	
	if($qry_count1 > 0)
	{
		mysqli_query($con, "DELETE FROM case_followers WHERE case_id = '$case_id' AND follower_id = '$follower_id'");
		
		header("Location: khojhai_view_missing.php?id=$case_id&err=4");
		exit();
	}	
	
	else
	{
		mysqli_query($con, "INSERT INTO case_followers (case_id, added_by_id, follower_id, followed_on, seen_by_adder) VALUES('$case_id', '$added_by', '$follower_id', NOW(), 'n')");
		
		header("Location: khojhai_view_missing.php?id=$case_id&err=5");
		exit();
	}			
}
