...
{
header("Location: khojhai_search2.php?q={$_GET['q']}&s=Search");
exit();
}
...
<title><?php echo $_GET['q']; ?> | KhojHai.com</title>
...
<a href='khojhai_search_offline.php' class='link_headr' title='Search'><u>Search</u></a>
...
<form action= "khojhai_search_offline2.php" method="GET" role="form" class="signup_form">

<!-------------- name of missing ------------->
<span class="label">Search By Name, Case ID, Gender Or State</span><br>

<input type = "text" id = "q" name = "q" class = "form-cntrl" placeholder = "Enter Name, Case ID, Gender Or State" required = "required" minlength = "1" maxlength = "100" 

pattern = "[A-Za-z0-9\s]{1,100}$" 

required title = "You Can Use Only English Characters, Digit Or Both. No Special Characters Are Allowed" spellcheck = "false" value="<?php echo $_GET['q']?>"/>

<br>

<!-------------- Submit Button ------------->

<input class = "btn" type = "submit" id = "s" name = "s" value = "Search" title = "Click Here To Search" /><br>
...
if($_GET['q'] == '0')
{
header("Location: khojhai_search_offline.php?err=1");
exit();	
}
...
if(empty($_GET['q']))
{
header("Location: khojhai_search_offline.php?error");
exit();	
}
...
$q = preg_replace('#[^a-zA-Z0-9\s]#i', '', $_GET['q']);

$s = preg_replace('#[^a-z]#i', '', $_GET['s']);
...
$qry2 = "SELECT * FROM cases WHERE (id = '$q' OR name LIKE '%$q%' OR gender = '$q' OR state = '$q') ORDER BY added_on DESC";
...
$PageControl .= '<a href="'.$_SERVER['PHP_SELF'].'?q='.$q.'&s='.$s.'&pg='.$previous.'" class = "pagination">Previous</a>&nbsp; ';
...
if($i > 0)
{
$PageControl .= '<a href="'.$_SERVER['PHP_SELF'].'?q='.$q.'&s='.$s.'&pg='.$i.'" class = "pagination">'.$i.'</a>&nbsp; ';
}
	}
         }
		
//Target Page Should Not Be clickable......
$PageControl .= '<a href="'.$_SERVER['PHP_SELF'].'?q='.$q.'&s='.$s.'&pg='.$pagenum.'" class = "pagination_active">'.$pagenum.'</a>&nbsp;';
...
if($pagenum != $last) 
{
        ...
        $PageControl .= '<a href="'.$_SERVER['PHP_SELF'].'?q='.$q.'&s='.$s.'&pg='.$next.'" class = "pagination">Next</a>';
}
     }
