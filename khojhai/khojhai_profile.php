...
<title>Profile | KhojHai.com</title>
...
if($_GET['id'] == $sess_id)
{
$pro_head_link = "<u>Profile</u>";	
}

else
{
$pro_head_link = "Profile";	
}

echo '<a href="khojhai_profile.php?id='.$sess_id.'" class="link_headr" title="'.$tmp_name.'">'.$pro_head_link.'</a>';
...
if(preg_match('/^[\s]{1,1000}$/', $_GET['id']))
{
header("Location: khojhai_home.php");
exit();	
}

if(!preg_match('/^[0-9]{1,1000}$/', $_GET['id']))
{
header("Location: khojhai_home.php");
exit();	
}
...
$user_profile_qry = mysqli_query($con, "select * from users where id = '$get_id' LIMIT 1");
...
echo "<a href='khojhai_profile.php?id=$get_id' class='profile_nav_link' style='border-bottom:medium solid #ff4040; text-decoration:none;'>Posted Cases</a>";

echo "<span class='nav_link_space'></span>";

echo "<a href='khojhai_profile_saved.php' class='profile_nav_link' style='border-bottom:medium solid #000; text-decoration:none;'>Saved Cases</a>";
...
$qry2 = "SELECT * FROM cases WHERE added_by_id = '$get_id' ORDER BY added_on DESC";
...
echo "<label style='font-size : 20px; font-family : Times New Roman, Times, Serif; color : #262626; background-color:#fff;'>No Records Found</label><br>";	
...
$PageControl .= '<a href="'.$_SERVER['PHP_SELF'].'?id='.$get_id.'&pg='.$previous.'" class = "pagination">Previous</a>&nbsp; ';
...
if($i > 0)
{
$PageControl .= '<a href="'.$_SERVER['PHP_SELF'].'?id='.$get_id.'&pg='.$i.'" class = "pagination">'.$i.'</a>&nbsp; ';
}
	}
         }
		
//Target Page Should Not Be clickable......
$PageControl .= '<a href="'.$_SERVER['PHP_SELF'].'?id='.$get_id.'&pg='.$pagenum.'" class = "pagination_active">'.$pagenum.'</a>&nbsp;';
...
if($pagenum != $last) 
{
        ...
        $PageControl .= '<a href="'.$_SERVER['PHP_SELF'].'?id='.$get_id.'&pg='.$next.'" class = "pagination">Next</a>';
}
     }
