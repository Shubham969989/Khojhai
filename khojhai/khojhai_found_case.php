...
$qry_count1 = mysqli_num_rows($qry1);
	...
	else
	{
		if($case_status == 'Open')
		{
        mysqli_query($con, "UPDATE cases SET status = 'Solved' WHERE id = '$case_id' LIMIT 1");
        }
		
		else
		{
		mysqli_query($con, "UPDATE cases SET status = 'Open' WHERE id = '$case_id' LIMIT 1");	
		}
		
		header("Location: khojhai_view_missing.php?id=$case_id&err=2");
		exit();
	}			
}
